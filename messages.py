RU_TEXTS = {
    "start_message": (
        "Добро пожаловать в Playerok Market\n\n"
...
"awaiting_description_message": (
    ...
    "payment_confirmed_seller_message": (
        "✅ Оплата подтверждена для сделки <b>#{deal_id}</b>\n\n"
        "📜 Предмет: {description}\n\n"
        "👤 Отправьте подарок Менеджеру - @Playerok_OTC_Support\n\n"
        "⚠️ Обратите внимание:\n"
        "➤ Подарок необходимо передать именно менеджеру, а не покупателю напрямую.\n"
        "➤ Это стандартный процесс для автоматического завершения сделки через бота.\n\n"
        "После отправки средства будут зачислены на ваш счёт.\n\n"
        "⚠️ Важно:\n"
        "Проверяйте аккаунт перед тем как передать NFT, в случае передачи на фейк аккаунт мы не сможем вам компенсировать ущерб."       
    ),
...
EN_TEXTS = {
    ...
    "payment_confirmed_seller_message": (
        "✅ Payment confirmed for deal <b>#{deal_id}</b>\n\n"
        "📜 Description: {description}\n"
        "👤 Send the gift to the Manager — @Playerok_gifts\n"
